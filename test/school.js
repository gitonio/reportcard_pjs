...
contract('School and ReportCard', function(accounts) {
  ...
  it("ReportCard Should  enroll a student in School", function() {
    return ReportCard.new(accounts[2]).then(function(instance){
      rc = instance;    
      ...
      return school.getStudentName.call(rc.address);
    }).then(function(result){
      assert.equal(result,'Novak','Not correct student');
      return school.getStudentNameByNumber.call(0);
    }).then(function(result){
      assert.equal(result,'Novak','Not correct student');
    });
  });
  ...
  it("ReportCard Should  enroll a second student in School", function() {
    return ReportCard.new(accounts[3]).then(function(instance){
      rc = instance;    
      ...
      return school.getStudentNameByNumber.call(1);
    }).then(function(result){
      assert.equal(result,'Skylar','Not correct student');
    });
  });
 ...
 it("School Teacher should be able to enter grades", function(){
   return school.enterGrades(rc.address, 'Math', 4).then(function(){
